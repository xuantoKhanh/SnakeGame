...
public class GamePanel extends JPanel implements ActionListener { //kế thừa thừ JPanel và interface ActionListener
    ...
    private String fileName = "SaveData1";


    GamePanel(){ //thiết lập cấu hình bằng cách kết hợp bên trong constructor
        try{
    ...
    public void startGame(){
        ...
        newApple(); //gọi hiển thị quả táo đầu tiên, gọi phương thức ở dưới
    }
    public void pause() {
        this.gameOn = true;
        timer.stop();
    ...
    public void draw(Graphics g){
        if(running) {
            ...
            g.drawString("Best: " + highscore1 , (SCREEN_WIDTH - metrics.stringWidth("Best: " + highscore1))/2, SCREEN_HEIGHT -10);
        }
        else{
            gameOver(g);
        }
    ...
    public void gameOver(Graphics g){
        ...
        g.drawString("Best: " + highscore1, (SCREEN_WIDTH - metrics1.stringWidth("Best: " +highscore1))/2, SCREEN_HEIGHT -10);
        //Game over text
        g.setColor(Color.red);
        ...
        if(applesEaten > highscore1){
            highscore1 = applesEaten;
        }
        sethighscore1();
        new ReStart();
    }
    ...
    private void loadhighscore1(){
        try{
            ...
            highscore1 = Integer.parseInt(reader.readLine());
            reader.close();
        }catch(Exception e){
            e.printStackTrace();
        }
